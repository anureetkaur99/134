...
SELECT fname, lname, dob, salary FROM student WHERE TRUNC(MONTHS_BETWEEN(SYSDATE, dob)/12) > 15;
SELECT ssn||','||LOWER(lname)||','||UPPER(fname) AS "Full_Name" FROM student;
SELECT fname, lname, dob, salary FROM student WHERE LOWER(lname) LIKE '%h%' OR LOWER(lname) LIKE '%a%';
